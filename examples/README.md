...
| [`mp_memory_coordinator/`](mp_memory_coordinator/) | Production Kustomize deployment of the standalone MP Memory Coordinator. Safely rebalances explicitly adopted Device-DAX allocations between MP servers, starting in observation-only mode with a durable journal and Kubernetes Lease. | Kubernetes cluster with 2+ Device-DAX worker nodes and an outside allocation service |
