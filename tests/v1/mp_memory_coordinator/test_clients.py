...
def test_outside_status_is_bare_mapping() -> None:
    ...
    assert str(rec.requests[0].url) == "http://alloc:8080/api/v2/apps/lmcache"
...
def test_deallocation_request_is_exact_and_response_fully_validated() -> None:
    ...
    assert str(sent.url) == "http://alloc:8080/api/v2/apps/lmcache/deallocations"
...
def test_allocation_request_is_exact_with_literal_values() -> None:
    ...
    sent = rec.requests[0]
    assert sent.method == "POST"
    assert str(sent.url) == "http://alloc:8080/api/v2/apps/lmcache/allocations"
    body = json.loads(sent.content)
