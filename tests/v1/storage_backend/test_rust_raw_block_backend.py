...
@pytest.mark.skipif(
    not os.environ.get("LMCACHE_RAW_BLOCK_URING_CMD_TEST_DEVICE"),
    reason="set LMCACHE_RAW_BLOCK_URING_CMD_TEST_DEVICE=/dev/ng0n1 to run",
)
def test_rust_raw_block_backend_uring_cmd_opens_explicit_device(
    memory_allocator,
    loop_in_thread,
):
    """Open the experimental NVMe io_uring_cmd path only for an explicit device."""
    dev_path = os.environ["LMCACHE_RAW_BLOCK_URING_CMD_TEST_DEVICE"]
    config = LMCacheEngineConfig.from_defaults(
        chunk_size=256,
        local_cpu=True,
        max_local_cpu_size=0.1,
        lmcache_instance_id="test_rust_raw_block_uring_cmd",
    )
    config.storage_plugins = []
    config.extra_config = {
        "rust_raw_block.device_path": dev_path,
