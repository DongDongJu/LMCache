...
from lmcache.multiprocess.transfer_context.pickle import EngineDrivenContextPickle
from lmcache.multiprocess.transfer_context.shm import EngineDrivenContextShm
...
if TYPE_CHECKING:
    ...
    from lmcache.multiprocess.modules.engine_driven_transfer import (
...
def _default_register_payload(
    ...
    """
    # First Party
    from lmcache.multiprocess.custom_types import RegisterEngineDrivenContextPayload
...
def test_create_transfer_context_force_lmcache_driven_mode() -> None:
    ...
    import lmcache.platform.cpu  # noqa: F401
...
def test_compute_kv_layout_empty_raises_value_error() -> None:
    ...
    # First Party
    from lmcache.multiprocess.transfer_context.base import compute_kv_layout
...
def server_module_factory(
    ...
    def _create(
        *,
        ...
        stack.enter_context(
            patch(
                "lmcache.multiprocess.engine_context.StorageManager",
                return_value=mock_storage,
            )
        )
        stack.enter_context(patch("lmcache.multiprocess.engine_context.TokenHasher"))
        session_cls = stack.enter_context(
            patch("lmcache.multiprocess.engine_context.SessionManager")
        )
        stack.enter_context(patch("lmcache.multiprocess.engine_context.get_event_bus"))
        stack.enter_context(
            patch(
                "lmcache.multiprocess.engine_context.ipc_key_to_object_keys",
...
def test_engine_context_shm_pool_info(
    ...
    with patch(
        "lmcache.distributed.config.current_device_spec",
        MagicMock(is_pin_supported=True),
    ):
        config = _make_storage_manager_config(**config_kwargs)

    with (
        patch("lmcache.multiprocess.engine_context.StorageManager"),
        patch("lmcache.multiprocess.engine_context.TokenHasher"),
        patch("lmcache.multiprocess.engine_context.SessionManager"),
        patch("lmcache.multiprocess.engine_context.get_event_bus"),
    ):
...
def test_server_prepare_store_releases_unused_reserved_write_locks(
    ...
    # First Party
    from lmcache.multiprocess.protocols.engine import PrepareStoreResponse
...
def test_gather_paged_kv_with_chunk_indices_subset() -> None:
    ...
    """
    # First Party
    from lmcache.multiprocess.transfer_context.base import gather_paged_kv_to_cpu
