...
from lmcache.distributed.internal_api import L2AdapterListener
from lmcache.distributed.l2_adapters import create_l2_adapter
from lmcache.distributed.l2_adapters.bigtable_l2_adapter import (
    BigtableL2Adapter,
...
def setup_bigtable_mocks():
    ...
    with (
        mock.patch(
            "lmcache.distributed.l2_adapters.bigtable_l2_adapter.BigtableDataClientAsync",
            FakeBigtableDataClientAsync,
        ),
        mock.patch(
            "lmcache.distributed.l2_adapters.bigtable_l2_adapter.google.auth.default",
...
class TestBigtableL2Adapter:
    ...
    def test_sharder_and_key_encoder(self):
        # First Party
        from lmcache.distributed.l2_adapters.bigtable_key_encoder import (
            BigtableL2KeyEncoder,
        )
        from lmcache.storage_backend.connector.bigtable_sharder import (
    ...
    def test_store_skips_large_writes_sharding_enabled(self):
        ...
        with mock.patch(
            "lmcache.distributed.l2_adapters.bigtable_l2_adapter._prepare_and_shard",
    ...
    def test_dual_mode_write(self):
        ...
        with (
            mock.patch(
                "lmcache.distributed.l2_adapters.bigtable_l2_adapter."
                "_prepare_and_shard",
    ...
    def test_unsharded_bulk_store_size_chunking(self):
        ...
        with (
            mock.patch(
                "lmcache.distributed.l2_adapters.bigtable_l2_adapter._prepare_bytes",
