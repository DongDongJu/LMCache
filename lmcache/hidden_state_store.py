...
the KV cache. It is constructed by :class:`~lmcache.cache_engine.LMCacheEngine`
when ``config.enable_hidden_state_cache`` is True and exposed as
``engine.hidden_state_store``. See ``docs/design/hidden_state_store.md``.
"""
...
from lmcache.token_database import TokenDatabase
