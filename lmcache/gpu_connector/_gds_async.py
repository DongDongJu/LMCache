...
(:mod:`lmcache.gpu_connector._cufile_async`) on NVIDIA, or the hipFile
wrapper (:mod:`lmcache.gpu_connector._hipfile_async`) on AMD ROCm. Both
...
:mod:`lmcache.gpu_connector.gds_context` imports this shim as ``ca`` and is
platform-agnostic.
...
if TYPE_CHECKING:
    # First Party
    from lmcache.gpu_connector import _cufile_async as _backend
elif torch.version.hip is not None:
    # First Party
    from lmcache.gpu_connector import _hipfile_async as _backend
else:
    # First Party
    from lmcache.gpu_connector import _cufile_async as _backend
