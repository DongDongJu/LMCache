...
from lmcache.gpu_connector.kv_format import (
    concrete_shape,
    describe_shape,
    detect_format,
    get_spec,
...
from lmcache.gpu_connector.kv_format.types import DiscoverableKVCache, LayoutHints
...
def assert_contiguous(tensor: torch.Tensor) -> None:
    ...
    in :class:`~lmcache.platform.cuda.ipc_wrapper.RawCudaIPCWrapper`).
...
def assert_layerwise_gpu_connector(gpu_connector: "GPUConnectorInterface"):
    ...
    # First Party
    from lmcache.gpu_connector import gpu_connectors, xpu_connectors
...
def normalize_kv_and_discover_format(
    ...
    Thin wrapper over
    :func:`lmcache.gpu_connector.kv_format.detect_format`; see that
...
def normalize_and_discover_per_layer_formats(
    ...
    Returns:
        ...
        ready for :func:`lmcache.kv_layer_groups.group_layers_by_identity`.
    """
