...
class RustRawBlockBackend(StoragePluginInterface):
    ...
    def _batched_get_prefix(
        ...
        metas = self._core.get_metadata_many([spec.encoded for spec in specs])

        prefix_specs = []
        prefix_metas = []
        for spec, meta in zip(specs, metas, strict=False):
            if meta is None:
                break
            prefix_specs.append(spec)
            prefix_metas.append(meta)
