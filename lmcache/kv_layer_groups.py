...
# First Party
from lmcache.distributed.api import AttnWindowDesc
...
if TYPE_CHECKING:
    # First Party
    from lmcache.gpu_connector.utils import DiscoverableKVCache
...
class KernelGroupInfo:
    ...
    :func:`~lmcache.gpu_connector.utils.get_group_data_ptrs`) and
...
class KVLayerGroupsManager:
    ...
    :mod:`lmcache.gpu_connector.utils`; this class only drives the
    ...
    def __init__(
        ...
        # lmcache.gpu_connector.__init__ → metadata → kv_layer_groups.
        # First Party
        from lmcache.gpu_connector.utils import (
            get_num_blocks,
...
def parse_kvcache_shape_spec(
    ...
    **Field semantics** (names aligned with ``EngineKVFormat``; see
    :func:`lmcache.gpu_connector.utils.get_engine_kv_shape_description`):
