...
:class:`~lmcache.platform.base_ipc_wrapper.DeviceIPCWrapper`
...
def _discover_wrappers_once() -> None:
    ...
    Walks ``lmcache.platform`` two levels deep for
    :class:`~lmcache.platform.base_ipc_wrapper.DeviceIPCWrapper`
    ...
    (so e.g. :class:`~lmcache.platform.cuda.ipc_wrapper.RawCudaIPCWrapper`
    is skipped in favour of
    :class:`~lmcache.platform.cuda.ipc_wrapper.CudaIPCWrapper`).
    ...
    *device_type* trigger a warning; the first one wins.
    ...
    with _DISCOVERY_LOCK:
        ...
        # First Party
        from lmcache.platform.base_ipc_wrapper import DeviceIPCWrapper
...
def reset_for_tests() -> None:
    ...
    clean slate (see ``tests/multiprocess/conftest.py``).
    """
    global _WRAPPERS_DISCOVERED
    _KV_WRAPPER_FACTORIES.clear()
