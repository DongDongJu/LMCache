...
* :class:`~lmcache.platform.cuda.cache_context.GPUCacheContext` --
  CUDA-backed.
* :class:`~lmcache.platform.cpu.cache_context.CPUCacheContext` --
...
:class:`~lmcache.platform.base_cache_context.BaseCacheContext`
...
:func:`lmcache.utils.subclass_discovery.discover_subclasses` and
...
def _discover_backends_once() -> None:
    ...
    Walks ``lmcache.platform`` two levels deep (``platform`` ->
    ...
    # First Party
    import lmcache.platform as platform_pkg
...
def _resolve_backend(device_type: str) -> type:
    ...
    if cls is None:
        raise ValueError(
            ...
            "Make sure ``lmcache.platform.<backend>.cache_context`` "
