...
This module sits next to :mod:`lmcache.platform.cuda.cache_context`
under :mod:`lmcache.platform` and provides the same public API as
:class:`~lmcache.platform.cuda.cache_context.GPUCacheContext` but
keeps all tensors on CPU. Stream / Event objects are provided by
:class:`~lmcache.platform.cpu.stub_cpu_device.StubStream` so
...
:mod:`lmcache.platform.cache_context`.
"""
...
from lmcache.kv_layer_groups import KVLayerGroupsManager
...
from lmcache.platform.cpu.stub_cpu_device import StubStream
...
class CPUCacheContext(BaseCacheContext):
    ...
    def __init__(
        ...
        # First Party
        from lmcache.platform.cuda.cache_context import (
