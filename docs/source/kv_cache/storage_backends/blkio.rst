...
- **Dual-mode support**: Works in non-MP mode via ``BlkioClient`` and in MP
  mode via the built-in ``BlkioL2Adapter``.
...
.. code-block:: text
   ...
   MP mode:
     StoreController / PrefetchController
           |
     BlkioL2Adapter (Python bridge)
       +- 3 eventfds (store, lookup, load)
       +- completion demux thread
       +- volatile key -> offset/size map and lock tracking
           |
...
The blkio extension is optional. LMCache builds it automatically when
``pkg-config blkio`` is available. Set ``BUILD_BLKIO=1`` to require it, or
``BUILD_BLKIO=0`` to force-disable it.
...
These test ``BlkioL2AdapterConfig`` parsing, validation, registry wiring, and
store -> lookup -> load -> unlock -> delete behavior with a fake native client.
...
2. **Volatile MP metadata** -- ``do_single_exists`` always returns ``false``.
   ``BlkioL2Adapter`` tracks written keys, offsets, sizes, and locks in memory
   so MP lookup/load works. This map is not checkpointed, so restart recovery is
   not supported for blkio MP L2 in this PR.
