...
```bash
curl -s $OUTSIDE_API_URL/v2/apps/lmcache | jq
...
curl -s $OUTSIDE_API_URL/v2/apps/lmcache | jq            # donor: [], receiver: [".../dax0.1"] on W2
...
curl -s -X POST $OUTSIDE_API_URL/v2/apps/lmcache/deallocations -H 'content-type: application/json' \
  -d '{"request_id":"cleanup-shrink-1","target_node":"'$W2_IP'","device_path":"'$DAX_ROOT'/dax0.1"}'
curl -s -X POST $OUTSIDE_API_URL/v2/apps/lmcache/allocations -H 'content-type: application/json' \
