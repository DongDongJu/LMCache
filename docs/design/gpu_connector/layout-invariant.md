...
> information via helpers in `lmcache/gpu_connector/utils.py` that
...
The layout logic lives in `lmcache/gpu_connector/kv_format/`. The
...
4. Add a row to the golden table in
   `tests/gpu_connector/test_kv_format_specs.py` and a detection
...
- **`lmcache/kv_layer_groups.py::KVLayerGroupsManager.__init__`** —
  ...
  `get_shape_desc`.
- **`lmcache/platform/cuda/cache_context.py::GPUCacheContext`** —
...
- **`lmcache/gpu_connector/gpu_connectors.py::VLLMPagedMemGPUConnectorV3._initialize_kv_cache_pointers`**
