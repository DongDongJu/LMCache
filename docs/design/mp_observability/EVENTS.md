...
(see `lmcache/mp_observability/errors.py`) every time the exception is
...
| EventType | Metadata keys | Types |
|---|---|---|
| `TRACE_CALL` | `qualname`, `args` | `str`, `dict[str, Any]` (codec-encoded; see `lmcache.mp_observability.trace.codecs`) |
