...
Event(
    ...
    metadata   = {
        "qualname": "lmcache.distributed.storage_manager.StorageManager.reserve_write",
...
`lmcache/distributed/api.py`. Keeping them in a leaf module means
...
The `replay` command accepts the full `lmcache/distributed/config.py`
...
4. **Codec registry** — new arg types slot in by calling
   ...
   in `lmcache/distributed/api.py` (or another leaf module) so
...
### Capture (PR1) — `tests/mp_observability/trace/`
