...
Location: `lmcache/platform/event_notifier.py`
(re-exported from the `lmcache.platform` package)
...
```python
# lmcache/platform/event_notifier.py
# (re-exported via `lmcache.platform` for short imports)
...
`hasattr(os, "eventfd")` in `lmcache/platform/event_notifier.py`
and re-exported from the `lmcache.platform` package — the single
...
Unit tests live in `tests/test_event_notifier.py`. They cover:
