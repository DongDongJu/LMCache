...
(`lmcache/distributed/transfer_channel/`): its public API, how a read flows
...
`lmcache.distributed.transfer_channel` (`__init__.py`).
...
2. **Provide a factory and register it** at module import time:
   ...
   ```python
   from lmcache.distributed.transfer_channel.factory import (
...
- Unit tests for the data types, factory registry, and global lifecycle:
  `tests/distributed/transfer_channel/test_api.py`,
...
- Integration tests for the nixl implementation (guarded by
  ...
  `tests/distributed/transfer_channel/test_nixl_impl.py`.
