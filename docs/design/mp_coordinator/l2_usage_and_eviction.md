...
Code: `lmcache/mp_coordinator/cache_control/` (coordinator side),
`lmcache/mp_coordinator/http_apis/cache_api.py` (REST endpoints),
`lmcache/mp_coordinator/schemas.py` (wire types),
`lmcache/multiprocess/http_server.py` (MP-server wiring).
...
- **``EncodedObjectKey``** — torch-free wire shape of ``ObjectKey`` (owned by
  ``lmcache.distributed.api``); ``chunk_hash`` is hex-encoded instead of raw
...
### QuotaManager (reused from ``lmcache.distributed.quota_manager``)
