...
Code: `lmcache/mp_coordinator/blend_directory.py`,
`lmcache/mp_coordinator/http_apis/blend_directory_api.py`,
`lmcache/multiprocess/modules/blend_coordinator.py`.

## Why

CacheBlend lookup today is **local to one mp-server**: the matcher
(`BlendTokenRangeMatcherV3`, `lmcache/multiprocess/modules/blend_v3.py`)
