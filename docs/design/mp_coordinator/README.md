...
1. `http_apis/quota_api.py` — a module-level `router` (FastAPI `APIRouter`).
   `create_app` auto-discovers it (via `lmcache/utils/router_discovery.py`,
...
(or, equivalently, `python -m lmcache.mp_coordinator`).
...
`lmcache/multiprocess/http_server.py`'s lifespan and configured by a
`CoordinatorConfig` (`lmcache/multiprocess/config.py`), built from
