...
| `lmcache/v1/distributed/l2_adapters/blkio_l2_adapter.py` | `BlkioL2AdapterConfig` + factory for MP mode |

## Building

The blkio extension is optional. It is built automatically only when
`pkg-config blkio` is available, can be required with `BUILD_BLKIO=1`, and can
be disabled with `BUILD_BLKIO=0`:

```bash
# Standard build (requires torch pre-installed; builds blkio if available)
pip install -e . --no-build-isolation

# Require blkio support
BUILD_BLKIO=1 pip install -e . --no-build-isolation
...
2. **Volatile MP metadata** — `do_single_exists` always returns `false`.
   `BlkioL2Adapter` tracks written keys, offsets, sizes, and locks in memory so
   MP lookup/load works, but restart recovery is not supported.
