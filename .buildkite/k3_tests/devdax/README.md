...
A project administrator must provision CPU queue `devdax-qemu`, install the image
and kernel, and set the PATH/image/kernel environment above. Paste
...
Use `buildkite-pipeline.yml` for a dedicated manual/nightly pipeline against
`LMCache/LMCache`, branch `dev`, schedule `0 2 * * *` UTC, with `auto`, `both` and
`kvm`. Verify L1-only, L2-only, combined, unrelated-path and explicit-off builds;
